...
pub fn random_default_setup() -> State<'static> {
    let mut state = State::new(&layout::DEFAULT, 2);
    // BoardState
    ...
    for (coord, landtile) in coord_landtile_pairs {
        state.board.hexes.set_value(&coord, match landtile {
            Some(res) => {
    ...
    for (path_coord, &porttile) in coord_porttile_pairs {
        for intersection_coord in path_coord.path_intersection().expect("Wrong path").iter() {
            state.board.harbors.set_value(intersection_coord, porttile)
